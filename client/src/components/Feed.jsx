  ...
  return (
    <div className="posts mt-2 flex flex-col gap-3">
      ...
      {posts.length !== 0 ? (
            ...
            >
              <div className="head flex justify-between items-center">
                <div className="flex gap-2 items-center">
                  <img
                    src={`${import.meta.env.VITE_API_BASE_URL}/assets/${post.userPicturePath}`}
                    alt="dp"
                    className="w-12 h-12 rounded-full object-cover cursor-pointer"
                    onClick={() => {
                      setshowimage(true);
                      setimage(
                        `${import.meta.env.VITE_API_BASE_URL}/assets/${post.userPicturePath}`
                      );
                    }}
                  />
                  <Link
                ...
                <div className="left-head">
                  {post.userId === User._id ? (
                    <div className="flex items-center gap-2">
                      ...
                      <img
                        ...
                        onClick={async () => {
                          ...
                          if (isConfirm) {
                            let res = await fetch(
                              `${import.meta.env.VITE_API_BASE_URL}/posts/${post._id}/delete`,
                              {
                                method: "DELETE",
                                headers: {
                    ...
                    <img
                      src={`${
                        friends.filter((friend) => friend._id === post.userId)
                          .length !== 0
                      ...
                      onClick={async () => {
                        let res = await fetch(
                          `${import.meta.env.VITE_API_BASE_URL}/users/${User._id}/${post.userId}`,
                          {
                            method: "PATCH",
                            headers: {
                              "Content-Type": "application/json",
              ...
              <p className={`my-2 text-sm ${editdes ? "hidden" : "block"}`}>
                {post.description}
              </p>
              <div
                className={`${
              ...
              >
                <input
                  className={`w-[85%] bg-transparent h-6 outline-none p-2 text-wrap${
                ...
                />
                <button
                  className="bg-sky-400 text-white p-2 px-4 rounded-full"
                  onClick={async () => {
                    seteditdes(!editdes);
                    let res = await fetch(
                      `${import.meta.env.VITE_API_BASE_URL}/posts/${post._id}/edit`,
                      {
                        method: "PATCH",
                        headers: {
                          "Content-Type": "application/json",
                    ...
                    console.log(upd);
                    if (res.status === 200) dispatch(setPost({ post: upd }));
                  }}
                >
                  {" "}
                  Save
              ...
              >
                {post.picturePath && (
                  ...
                  >
                    <img
                      src={`${import.meta.env.VITE_API_BASE_URL}/assets/${post.picturePath}`}
                      alt="post"
                ...
                {post.videoPath && (
                  <div className={`w-full flex items-center bg-black`}>
                    ...
                    >
                      <source
                        src={`${import.meta.env.VITE_API_BASE_URL}/assets/${post.videoPath}`}
                      />
              ...
              >
                <div className="add-comment my-3 flex gap-3">
                  <input
                    className="w-full h-10 p-3 bg-gray-100 rounded-2xl"
                    type="text"
                    value={comment}
                  ...
                  <button
                    ...
                    onClick={async () => {
                      let res = await fetch(
                        `${import.meta.env.VITE_API_BASE_URL}/posts/${post._id}/comment`,
                        {
                          method: "PATCH",
                          headers: {
                            "Content-Type": "application/json",
        ...
        <div className="flex justify-center items-center text-red-500 text-lg font-semibold">
          No Posts Available!!!
        </div>
      )}
    </div>
