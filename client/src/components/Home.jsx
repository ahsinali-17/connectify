...
const Home = () => {
  ...
  const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/posts/getposts/${User._id}`, {
      method: "GET",
      headers: {
        Authorization: token,
      },
    ...
    >
      ...
      <div className="flex gap-2">
        <div
          ...
          } max-h-[75vh] overflow-y-auto shadoww`}
        >
          <Dashboard User={User} />
        </div>
        ...
        <div
          className={`lg:w-[25%] h-[88vh] lg:flex ${
            navigation === "friends" ? "flex" : "hidden"
          } w-full rounded-lg ${
            mode === "dark" ? "bg-gray-700" : "bg-white"
          } overflow-y-auto shadoww`}
        >
          <FriendSection />
        </div>
      </div>
